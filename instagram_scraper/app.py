...
class InstagramScraper(object):
    ...
    def scrape(self, starttime=0, endtime=0, executor=concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS)):
        ...
        try:
            for username in self.usernames:
                ...
                try:
                    ...
                    self.get_media(dst, executor, future_to_item, user, starttime, endtime)
    ...
    def get_media(self, dst, executor, future_to_item, user, starttime=0, endtime=0):
        ...
        for item in tqdm.tqdm(self.query_media_gen(user), desc='Searching {0} for posts'.format(username),
            ...
            # --------------#
                ...
                if self.has_selected_media_types(item) and self.is_new_media(item):
                    if (starttime == 0 and endtime == 0) or starttime < item['taken_at_timestamp'] < endtime:
                        item['username']=username
                        future = executor.submit(self.worker_wrapper, self.download, item, dst)
                        future_to_item[future] = item
                        self.posts.append(item)
